...
class DanmakuState internal constructor(
    ...
    private val renderer: DanmakuRenderer
) {
    ...
    private val consumed = mutableSetOf<DanmakuInfo>()
    ...
    fun draw(
        ...
        while (iterator.hasNext()) {
            ...
            if (!shouldRemove) {
                renderer.draw(drawScope, danmaku.text, x, danmaku.baseline, danmaku.style)
            }
    ...
    private fun buildActiveDanmaku(
        ...
        val style = DanmakuTextStyle(
            fontSize = actualTextSize,
            color = applyOpacity(info.argbColor, config.opacity),
            shadowRadius = config.outlineRadius,
            shadowColor = applyOpacity(0xFF000000.toInt(), config.opacity)
        )

        val text = info.text
        val metrics = renderer.measure(text, style)
...
private fun rememberFrameClockMillis(): State<Long> =
    produceState(initialValue = elapsedRealtimeMillis()) {
        while (true) {
